...
def fit_gaussians(show = False):
    ...
    if (show) :
        for fit in gaussian_fits:
            ...
            cell_id = fit['cell_id']
            shank_id = fit.get('shank_id', shank) # Fallback for older versions of 'fit' dict
            ...
            if fit['params'] is not None:
                ...
                im1 = ax1.imshow(original_map, cmap='viridis', origin='lower', vmin=vmin, vmax=vmax)
                ...
                ax2.imshow(fitted_gaussian_map, cmap='viridis', origin='lower', vmin=vmin, vmax=vmax)
                ...
                fig.colorbar(im1, ax=[ax1, ax2], label='Firing Rate (Hz)', shrink=0.7)
                ...
                vacant_mask = (original_map == -1) | np.isnan(original_map)
                original_map[vacant_mask] = np.nan
                im = ax.imshow(original_map, cmap='viridis', origin='lower')
                ax.set_xlabel('X Bins')
                ax.set_ylabel('Y Bins')
                fig.colorbar(im, ax=ax, label='Firing Rate (Hz)', shrink=0.8)
