...
def create_bins(bin_size_cm = temp_bin_size_cm, SESSION = np.nan, arena_diameter_cm = arena_curr_diam):
    ...
    for i in range(num_bins_per_axis):
        for j in range(num_bins_per_axis):
            ...
            # For defected sessions, the arena is cutted
            elif SESSION == "mP31_11" and y_cm > 35:
                bins[i, j] = OUTSIDE_FLAG
            elif SESSION == "mP31_20" and y_cm > 33.2:
                bins[i, j] = OUTSIDE_FLAG
...
def create_reflection_padded_matrix(data_matrix, bins_grid, kernel_size):
    ...
    # For outside bins, change to outside value
...
def change_grid (map, new_bin_size, ARENA_DIAMETER, vacants, SESSION, curr_bin_size = 1):
    ...
    # Create the new grid
    new_grid = create_bins(new_bin_size, SESSION, arena_diameter_cm= ARENA_DIAMETER)
    
    return new_map, new_vacants, new_grid

def rates_map(bin_size, cell_id, x_values, y_values, tet_res, clu, SESSION, 
              KERNEL_SIZE = 7, ARENA_DIAMETER = arena_curr_diam, POS_SAMPLING_RATE = 1250):
    res = data.get_cell_spike_times(clu, tet_res, cell_id)

    # If the unit does not exist for this shank
    if (res.size == 0):
        return None, None
    
    # 1. Create the base grid
    init_bin_size = 1
    bins_grid_1cm = create_bins(init_bin_size, SESSION, arena_diameter_cm=ARENA_DIAMETER)
    ...
    # 6. Change bins size
    spike_map_sized, new_vacants, bins_grid = change_grid(spike_map_smoothed, bin_size, ARENA_DIAMETER, vacants, SESSION, init_bin_size)
    time_map_sized, new_vacants, bins_grid = change_grid(time_map_smoothed, bin_size, ARENA_DIAMETER, vacants, SESSION, init_bin_size)
...
def smooth_map(data_array, x_values, y_values,vacants, SESSION, BIN_SIZE = 10,time_data = False, from_mua = False,
                KERNEL_SIZE=7, ARENA_DIAMETER=arena_curr_diam, POS_SAMPLING_RATE = 1250):
    init_bin_size = 1 
    bins_grid_1cm = create_bins(init_bin_size, SESSION, arena_diameter_cm=ARENA_DIAMETER)
    gaussian_kernel = create_gaussian_kernel(size=KERNEL_SIZE)
    ...
    data_matrix_sized, new_vacants, bins_grid = change_grid(data_matrix_smoothed, BIN_SIZE, ARENA_DIAMETER, vacants, SESSION, init_bin_size)
...
def occupancy_map (SESSION, bin_size, DTYPE = np.int16, N_CHANNELS = 136, X_CHANNEL = 124, Y_CHANNEL = 125, 
    ...
    # 6. Change bins size
    occupancy_map_sized, new_vacants, bins_grid = change_grid(occupancy_map_smoothed, bin_size, ARENA_DIAMETER, vacants, SESSION, init_bin_size)
