...
'''
this test file runs prediction analysis over different test durations and bin sizes,
...
# Y_CHANNEL = 125
# N_CHANNELS = 136
CELL_ID = np.array([7,2,11,11,5,8,3,7,4,6,9,1,10,4,2,8,6,3,5,7,1,9,2,4,10,6,3,11,5,8,1])#([2,16,12,2,2,11,2,21,4])
TETRODE_ID = np.array([1,1,2,8,1,1,1,2,2,2,3,3,3,4,4,5,5,5,6,6,6,7,7,8,8,9,9,10,10,11,11])#([4,1,12,12,2,2,1,1,3])
KERNEL_SIZE = 7
...
##--- Plot Dual Animation of Raw and Smoothed Positions ---
# data.plot_dual_animation(
#         x_values,
#         y_values,
#         x_smooth,
#         y_smooth,
#         start_time_minutes=105,
#         duration_seconds=10
#     )
...
for tet, cell in zip(TETRODE_ID, CELL_ID):
    ...
    tet_res, clu = data.get_tetrode_spike_times(f"data/{SESSION}/{SESSION}.clu", f"data/{SESSION}/{SESSION}.res", tet, POS_SAMPLING_RATE, RES_SAMPLING_RATE)
...
durations = np.arange(0.5, 40, 0.5)
accuracies = []
chance_levels = []
errors = []
SEM = []
...
ax1.plot(durations, accuracies, color='b', label="Accuracy" )
...
ax1.set_title(f'Prediction Accuracy vs Test Duration with {CELL_ID.size} Cells, Bin size {BIN_SIZE}cm, Session {SESSION}')
ax1.set_ylim(0, 100)
ax1.grid(True)
...
ax1.plot(bin_sizes, chance_levels, marker = 'o', linestyle='--', color='r', label="Chance")
ax1.fill_between(bin_sizes,  bottom,  top,  color='red',  alpha=0.3, zorder=1, label='SEM')
ax1.set_ylim(0, 100)
ax1.set_xlabel('Bin Size (cm)')
ax1.set_ylabel('Prediction Accuracy (%)')
ax1.set_title(f'Prediction Accuracy vs Bin Size with {CELL_ID.size} Cells, Duration {test_duration}s,  Session {SESSION}')
ax1.grid(True)

# Create a secondary y-axis that shares the same x-axis
ax2 = ax1.twinx()  
ax2.plot(bin_sizes, errors, linestyle='-', color='g', marker='o', label="Error (cm)")
ax2.set_ylabel('Error (cm)')
