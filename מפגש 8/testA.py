...
zeros = (spike_map_raw == 0).astype(int)
# ===================================================================
# Visualization
# ===================================================================
...
plt.imshow(heatmaps.remove_vacants(time_map_raw, vacants, True), cmap='jet', origin='lower')
plt.colorbar(label='Firing Rate [spikes/sec]')
plt.title(f"Raw time map")
...
plt.figure()
plt.imshow(heatmaps.remove_background(time_map_covered, bins_grid), cmap='jet', origin='lower')
plt.colorbar(label='rate')
plt.title(f"Time map with covered vacants")        
plt.xlabel("X Bins")
plt.ylabel("Y Bins")
plt.show()
"""
